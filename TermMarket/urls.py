...
urlpatterns = [
    ...
    url(r'^admin/term_market/enrollment/solver/check/(?P<task>[a-zA-Z0-9\-]+)/$',
        'term_market.solver.solver_task_check', name='solver_check'),
    url(r'^admin/term_market/enrollment/(?P<enrollment>[0-9]+)/solver/run/$',
        ManualSolverRunView.as_view(), name='run_solver'),
    url(r'^admin/term_market/enrollment/(?P<enrollment>[0-9]+)/solver/running/(?P<task>[a-zA-Z0-9\-]+)/$',
